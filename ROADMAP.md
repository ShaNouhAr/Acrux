...
- [x] **Modèles 3D** (`acrux-features/three_d`, `acr 3d`, `acr create --3d`) : annotations `/3D` listées avec leur vue par défaut (`/3DV`, `/C2W`, `/CO`, `/FOV`, `/BG`), **U3D lu de zéro** — décodeur arithmétique à contextes de la norme ECMA-363, blocs de nœuds, maillage de base, nuanceurs et matériaux —, rendu maison (projection, tampon de profondeur, lampe frontale) et manipulation à la souris dans l'application ; PRC reconnu mais non lu, raffinement progressif et textures à faire
...
- [~] Fenêtre native Win32 (`acrux-app/platform`), tampon de pixels avec sous-vues, DPI par moniteur (dès la création), glisser-déposer, presse-papiers, curseurs, ouverture d'URL ; toolkit interne (`ui/text` texte d'interface via acrux-fonts + acrux-graphics, `ui/theme` sombre/clair, `ui/input` champ de saisie, `ui/icons` icônes vectorielles, `ui/toolbar` barre d'outils avec champ de page et liste du zoom (`ui/zoompicker` : niveaux, ajustements, niveau tapé), annuler / rétablir grisés selon l'historique, bascules allumées quand ce qu'elles montrent est affiché, boutons secondaires effacés dans une fenêtre étroite, `ui/palette` palette de commandes, barre d'état cliquable (page, zoom, disposition ; `viewer/status.rs`), info-bulles des boutons — libellé et raccourci repris de la palette, source unique, sans info-bulle sur un bouton grisé — et des onglets — chemin complet, modifications non enregistrées)
...
- [x] Recherche complète (`text::search`, `viewer/search.rs`, `acr find`) : un seul moteur pour les surlignages, le remplacement et la ligne de commande ; « Aa » (respecter la casse) et « Mot entier » ; occurrences à cheval sur deux lignes d'un paragraphe, césures refermées ; apostrophes, espaces insécables et ligatures repliés ; boutons précédente / suivante / fermer, « 3 sur 12 », « Aucun résultat » ; F3 / Maj+F3 (la colonne d'outils passe à Maj+F4) ; Ctrl+F reprend la sélection et sélectionne la requête ; première occurrence à partir de la page affichée ; parcours pendant la peinture (un réveil par image). À faire : recherche dans tout un dossier, expressions régulières, repli des accents en option, liste de toutes les occurrences avec leur contexte
- [x] Rechercher et remplacer (Ctrl+H) : second champ sous la recherche, boutons « Remplacer » et « Tout remplacer », remplacement chirurgical qui garde la police, le corps et la couleur de chaque occurrence (`render_worker::replace_all`, test `replace_corpus`) ; casse et mot entier respectés (`replace_all_with`), occurrences à cheval sur deux lignes laissées et annoncées
...
- [x] Palette de commandes (Ctrl+Maj+P, `ui/palette.rs`) : toutes les commandes de l'application avec leur raccourci, filtrage par fragment, par sous-séquence et par synonyme, accents optionnels dans les deux sens ; tient lieu de barre de menus et de documentation vivante des raccourcis ; ordre d'usage et commandes récentes en tête (retenues dans les préférences), lettres trouvées éclairées, liste qui défile avec la sélection (pages, bouts, molette, ascenseur), libellés et raccourcis traduits, recherche dans la langue affichée
- [x] Molette : jamais le document sous une palette, une invite, une capture ou un sélecteur ouverts (elle va à leur liste) ; position convertie des coordonnées d'écran (`ScreenToClient`), si bien que le panneau, la colonne d'outils et un modèle 3D la reçoivent où que soit la fenêtre
...
- [~] Organisation des pages : insérer, supprimer, extraire, réordonner, pivoter, fusionner (faits, `acr rotate/delete/reorder/extract/merge`) ; dans l'application : réordonner par glisser-déposer dans les vignettes (repère d'insertion), Enregistrer / Enregistrer sous, confirmation avant perte de modifications
  - [x] **Organiser les pages** (`viewer/organize.rs`, `ui/panel.rs`, `acrux-features/pages.rs`) : sélection multiple des vignettes (`Ctrl+clic`, `Maj+clic`, `Maj+flèches`, `Ctrl+A`), voile d'accent ; pivoter, supprimer, dupliquer, extraire et glisser **un bloc** en une seule modification ; page vierge avant / après au format de la voisine ; remplacer des pages par celles d'un autre fichier en gardant l'objet de la page ; menu du clic droit des vignettes
  - [x] **Fractionner** (`pages::split`, `acr split`) : par nombre de pages, signets de premier niveau, taille maximale, une page par fichier ; écrit par le fil de rendu, sans jamais écraser ; `acr extract --each`, `acr insert-blank`, `acr replace`
  - [x] Extraction propre : un lien vers une page non extraite ne l'embarque plus (`Importer::map_ref` / `drop_ref`), une page dupliquée a ses propres annotations, les calques suivent
  - [x] **Combiner et insérer depuis l'application** (`viewer/combine.rs`, `viewer/insert.rs`, `ui/combine.rs`, `ui/insertpages.rs`) : fenêtre « Combiner des fichiers » (ajout multiple, dépôt, ordre au glisser ou au clavier, signets, formulaires ; combinaison sur un fil, résultat dans un onglet non enregistré), feuille « Insérer des pages » (pages et position), source d'insertion **en mémoire** (l'annulation ne relit plus le fichier), dépôt de plusieurs fichiers (un onglet chacun, ou insertion à l'endroit visé des vignettes) ; `acr insert`
  - [x] Formulaires fusionnés à l'insertion et à la combinaison (`pages::InsertOptions`, `forms::merge_acroform`) : champs renommés en cas d'homonymie, signatures retirées ; destinations nommées rendues explicites à la copie (`navigation::named_to_explicit`)
  - [ ] Recadrer ; renuméroter les champs d'une page dupliquée ; fusionner le formulaire à l'extraction
...
- [~] Annotations : inventaire, création avec apparences — carré, surlignage, **soulignement, texte barré, soulignement ondulé** (une annotation par page, un quadrilatère par ligne), **signe d'insertion** (`/Caret`), **remplacement** (barré et signe groupés, §12.5.6.2), note, lien, **ellipse, ligne et flèche** (terminaisons `/LE`), **polygone, ligne brisée, encre** (traits simplifiés et lissés), **zone de texte** (`/FreeText` en police standard, `/DA`, `/DS`, cadre, fond, plusieurs lignes) et **légende** (`/IT /FreeTextCallout`, `/CL`, `/RD`), avec épaisseur et opacité (`/CA`) —, identifiant `/NM` et date de création tirés une fois et rejoués à l'identique, suppression d'un groupe entier (`acr annots/annotate`, `annots -v`) ; dans l'application : **barre des commentaires** (`ui/modebar.rs`), surligner (`H`), souligner (`U`), barrer, onduler, insérer, remplacer, poser une note (`N`), **dessiner** (rectangle, ellipse, ligne, flèche, crayon, avec aperçu et `Maj`) et **écrire** (zone de texte et légende tapées sur place ; `viewer/draw.rs`), réglages dans la barre, un seul `Ctrl+Z` par geste ; apparence par défaut synthétisée pour les annotations sans `/AP` (carré, cercle, ligne, encre, polygone, surlignage, soulignement, barré, ondulé, signe d'insertion, texte libre) ; **gestion des commentaires** : sélection à la souris (les nôtres et celles des autres logiciels), déplacement, redimensionnement, suppression (`Suppr`, fil compris), barre de propriétés (couleur, fond, épaisseur, opacité, texte ; apparence régénérée, `annotations::{edit, appearance}`), bulle avec **réponses** (`/IRT`) et **statuts** (`/State`, `/StateModel`, case cochée ; `annotations::review`), panneau trié, filtré et cherché, menu contextuel, `acr annot-set/annot-remove/reply` ; **tampons** d'Acrobat (`rubber_stamp`, `acr stamp`) ; FDF/XFDF à faire
...
- [~] Annuler / rétablir dans l'application (`Ctrl+Z` / `Ctrl+Y`, boutons de la barre d'outils, d'abord étape par étape dans le bloc en cours de saisie) : l'historique des modifications est rejoué depuis le fichier, ce qui évite d'avoir à inverser chaque opération ; historique de versions et annulation dans la bibliothèque à faire
...
- [x] **Zoom comme dans Acrobat** (`acrux-app/src/viewer/zoom.rs`, `ui/zoompicker.rs`) :
  Ctrl+molette zoome **vers le pointeur** (le point visé reste sous la souris ; fractions de cran
  d'un pavé tactile additionnées), les autres zooms et les ajustements gardent le centre de la vue.
  La case du zoom se déroule (niveaux, ajustements, niveau tapé), la barre d'état aussi. Pendant le
  rendu à la nouvelle échelle, la page est étirée depuis l'image précédente au lieu de repasser au
  blanc. Zoom plafonné pour qu'une page rendue tienne dans 256 Mo ; **reste** : le rendu par tuiles,
  qui lèvera ce plafond (Acrobat va jusqu'à 6400 %). L'historique de la vue (Alt+←) est fait.
...
- [x] **Barre des commentaires**, comme la barre « Commenter » d'Acrobat (`ui/modebar.rs`,
  `COMMENT_TOOLS` dans `viewer.rs`) : surligner, souligner, barrer, souligner d'un trait ondulé,
  insérer du texte, remplacer le texte, poser une note, en boutons d'icône avec info-bulle ;
  recliquer l'outil revient à la sélection, Échap éteint l'outil puis ferme la barre. Le panneau
  des commentaires nomme chaque type dans la langue de l'interface, montre un remplacement une
  seule fois et mène au passage commenté.
- [x] **Dessiner et écrire** dans la même barre (`viewer/draw.rs`, `annotations::{shapes, freetext}`) :
  rectangle, ellipse, ligne, flèche, crayon à plusieurs traits, zone de texte et légende. Le geste
  ...
  l'annotation). À faire : texte hors WinAnsi dans une zone (police incorporée).
- [x] **Tampons et images** (`acrux-features/rubber_stamp.rs`, `edit_objects::add_image`,
  `viewer/stamps.rs`, `ui/stamps.rs`, `acr stamp` / `add-image`) : les douze tampons d'Acrobat en
  annotations `/Stamp` à l'apparence vectorielle (double cadre arrondi, fond teinté, capitales),
  redressés sur une page tournée ; tampons **dynamiques** (auteur, date et heure du fuseau local,
  tirés au moment de la pose et rejoués à l'identique) et tampons **image** ; outil « Tamponner »
  dans la barre des commentaires, sélecteur aux vignettes rendues par le moteur, aperçu sous le
  pointeur, tampon sélectionné après la pose (déplacement, agrandissement aux proportions
  gardées). **Ajouter une image** depuis un fichier ou le presse-papiers (format « PNG »,
  `CF_DIBV5`, `CF_DIB` ; `platform::dib_to_bmp`) : un flux ajouté à `/Contents`, sans réécrire
  l'existant, l'image aussitôt prise en main par « Modifier les objets ». **Nouveau PDF** vierge
  ou depuis le presse-papiers. À faire : tampons personnalisés enregistrés (catalogue), insérer
  des pages depuis le presse-papiers.
- [x] **Gérer les commentaires** (`viewer/comments.rs`, `ui/annotbar.rs`, `ui/bubble.rs`) : un
  clic sélectionne une annotation (cadre, poignées), glisser la déplace (fantôme rendu par
  `acrux_render::render_annotation`), les poignées la redimensionnent, `Suppr` la supprime ;
  barre de propriétés flottante (nuancier et listes des outils de dessin) ; bulle d'une note avec
  auteur, date, statut, réponses et « Répondre » ; panneau « Notes » trié (page, auteur, date,
  type), filtré, cherché, avec statut et case ; menu du clic droit ; les pages gardent leur image
  le temps d'être refaites. Chaque geste est une `EditOp` (`AnnotSet`, `AnnotRemove`,
  `AnnotReply`, `AnnotState`) et s'annule d'un `Ctrl+Z`. À faire : poignées sur une page tournée,
  sélection de plusieurs annotations, glisser une ligne par ses extrémités.
...
- [~] **Menus contextuels** (clic droit, touche « menu », Maj+F10 ; `ui/menu.rs`, `viewer/context.rs`) :
  ...
  Menu d'un commentaire (répondre, texte, statut, case, supprimer).
  Les vignettes du menu agissent sur toute la sélection.
  À faire : menus d'un champ de formulaire précis et du
  mode « Modifier le PDF » ; info-bulles détaillées,
...
- [x] **Édition d'images et d'objets** (`acrux-features/edit_objects/`, `acr objects` /
  ...
  l'image, aligner plusieurs objets sur leur enveloppe, **poser une image neuve** (`add_image`).
...
- [~] **Combinaison** (`acrux-features/create/combine.rs`, `acr combine`, fenêtre « Combiner des
  fichiers » de l'application) : PDF, images (PNG, JPEG, BMP, GIF, TIFF multipage) et fichiers
  texte ou Markdown réunis en un document, nature devinée sur les octets de tête puis l'extension ;
  champs de formulaire gardés (renommés s'ils existent déjà, `--no-forms` pour s'en passer) ;
  PDF protégé ouvert avec son mot de passe ;
  ...
  **À faire** : portfolios (`/Collection`), entrelacement des pages, tri et rotation par fichier,
  reprise des signets d'origine sous le signet de chaque fichier, vignette de la première page dans
