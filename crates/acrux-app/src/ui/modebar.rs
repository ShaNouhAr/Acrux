...
impl ModeBar {
    ...
    pub fn mouse_move(&mut self, x: i32, y: i32) -> bool {
        ...
        let changed = over != self.hover || tool != self.hover_tool;
        self.hover = over;
    ...
    /// bouton par outil (`active` en accent), la consigne de l'outil en cours
    /// — ou, sans outil, comment s'en servir — et « Terminer ».
    ///
    /// Quand la fenêtre est étroite, la consigne se rogne d'abord, puis le
    /// titre s'efface : les boutons, eux, restent tous atteignables.
    #[allow(clippy::too_many_arguments)] // tout le contexte de dessin, rien de gardé
    pub fn paint_tools(
        &mut self,
        frame: &mut Frame<'_>,
        ...
        let buttons_w = (ctl + gap) * tools.len() as i32 - gap;
        let mut left = pad;
        if pad + title_w + s(16.0) + buttons_w + s(16.0) <= done_x {
            icons::draw(
                frame,
                raster,
                Icon::Comment,
