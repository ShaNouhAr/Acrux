//! Langue de l'interface : français ou anglais.
//!
//! Le texte de l'interface est écrit **en français dans le code** — c'est la
//! langue du projet, et une clé qui se lit vaut mieux qu'un identifiant qu'il
//! faut aller chercher. La traduction est donc une table `français → anglais`
//! consultée à l'affichage ; ce qui n'y figure pas reste en français, ce qui
//! est une dégradation lisible plutôt qu'une clé nue à l'écran.
//!
//! La langue effective est un entier partagé : la lire coûte une instruction,
//! ce qui permet d'appeler [`tr`] partout sans y penser, y compris dans une
//! boucle de dessin.

use std::sync::atomic::{AtomicU8, Ordering};

/// Langue choisie dans les préférences.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum Lang {
    /// Celle du système.
    #[default]
    Auto,
    /// Français.
    French,
    /// Anglais.
    English,
}

impl Lang {
    /// Clé écrite dans les préférences.
    #[must_use]
    pub fn key(self) -> &'static str {
        match self {
            Lang::Auto => "auto",
            Lang::French => "fr",
            Lang::English => "en",
        }
    }

    /// Langue d'une clé de préférences.
    #[must_use]
    pub fn from_key(key: &str) -> Lang {
        match key {
            "fr" => Lang::French,
            "en" => Lang::English,
            _ => Lang::Auto,
        }
    }

    /// Nom affiché.
    #[must_use]
    pub fn label(self) -> &'static str {
        match self {
            Lang::Auto => "Système",
            Lang::French => "Français",
            Lang::English => "English",
        }
    }
}

/// Langue effective : 0 = français, 1 = anglais.
static CURRENT: AtomicU8 = AtomicU8::new(0);

/// Choisit la langue effective. `system` est celle du système, employée
/// quand le réglage vaut « auto ».
pub fn apply(choice: Lang, system: Lang) {
    let effective = match choice {
        Lang::Auto => system,
        other => other,
    };
    CURRENT.store(u8::from(effective == Lang::English), Ordering::Relaxed);
}

/// Vrai si l'interface est en anglais.
#[must_use]
pub fn english() -> bool {
    CURRENT.load(Ordering::Relaxed) == 1
}

/// Traduit un texte d'interface écrit en français.
///
/// Rend la chaîne d'origine quand la langue est le français, ou qu'aucune
/// traduction n'est connue.
#[must_use]
pub fn tr(french: &'static str) -> &'static str {
    if english() {
        to_english(french)
    } else {
        french
    }
}

/// Traduction anglaise d'un texte, quelle que soit la langue effective ; le
/// texte d'origine si la table ne le connaît pas.
///
/// C'est le cœur de [`tr`], sans l'état global : la palette de commandes s'en
/// sert pour chercher dans les libellés de la langue qu'elle a lue à son
/// ouverture, et ses épreuves ne dépendent pas de celles d'ici, qui changent
/// la langue de tout le programme pendant qu'elles tournent.
#[must_use]
pub fn to_english(french: &'static str) -> &'static str {
    match TABLE.binary_search_by(|(fr, _)| (*fr).cmp(french)) {
        Ok(i) => TABLE[i].1,
        Err(_) => french,
    }
}

/// Traduit un texte à trous : chaque `{}` est remplacé, dans l'ordre, par les
/// valeurs données.
///
/// `format!` demande un littéral ; une phrase traduite n'en est pas un. Cette
/// fonction fait le remplacement à la main, ce qui permet aux deux langues
/// d'avoir leurs trous dans le même ordre.
#[must_use]
pub fn trf(french: &'static str, values: &[&str]) -> String {
    let mut out = String::with_capacity(french.len() + 16);
    let mut rest = tr(french);
    for value in values {
        match rest.split_once("{}") {
            Some((before, after)) => {
                out.push_str(before);
                out.push_str(value);
                rest = after;
            }
            None => break,
        }
    }
    out.push_str(rest);
    out
}

/// Table de traduction, **triée par le français** (une recherche
/// dichotomique la consulte). Un test vérifie l'ordre et l'absence de
/// doublons : une table désordonnée rendrait des traductions au hasard.
static TABLE: &[(&str, &str)] = &[
    (" (permissions retouchées : les plus strictes s'appliquent)", " (permissions tampered with: the strictest apply)"),
    (" — fichiers non protégés", " — files not protected"),
    (" — {} partie(s) dépassent la taille maximale (une page seule plus grosse)", " — {} part(s) exceed the maximum size (a single larger page)"),
    ("(sans texte)", "(no text)"),
    ("1 document récent", "1 recent document"),
    ("Accepté", "Accepted"),
    ("Accueil", "Home"),
    ("Accueil — Échap ou la maison pour revenir au document", "Home — Esc or the house to return to the document"),
    ("Acrux cherche une version plus récente au démarrage, au plus une fois par jour. Rien n'est installé sans votre accord.", "Acrux looks for a newer version at startup, at most once a day. Nothing is installed without your consent."),
    ("Acrux est à jour.", "Acrux is up to date."),
    ("Acrux ouvre les PDF et les images (PNG, JPEG, BMP, GIF, TIFF). Ignoré(s) :\n\n{}", "Acrux opens PDFs and images (PNG, JPEG, BMP, GIF, TIFF). Skipped:\n\n{}"),
    ("Acrux {} est disponible.", "Acrux {} is available."),
    ("Afficher le panneau latéral", "Show side panel"),
    ("Ajouter", "Add"),
    ("Ajouter des fichiers", "Add files"),
    ("Ajouter des fichiers…", "Add files…"),
    ("Ajouter du texte", "Add text"),
    ("Ajouter mon nom et la date", "Add my name and the date"),
    ("Ajouter une image", "Add image"),
    ("Ajouter une signature", "Add a signature"),
    ("Ajustement automatique", "Automatic fit"),
    ("Ajuster", "Adjust"),
    ("Ajuster à la largeur", "Fit width"),
    ("Ajuster à la page", "Fit page"),
    ("Aller à une page (numéro ou étiquette)", "Go to a page (number or label)"),
    ("Annuler", "Cancel"),
    ("Annuler la dernière action", "Undo the last action"),
    ("Annuler le trait", "Undo stroke"),
    ("Annulé", "Cancelled"),
    ("Aplatir", "Flatten"),
    ("Aplatir le formulaire", "Flatten the form"),
    ("Aplatir le formulaire ?", "Flatten the form?"),
    ("Apparence", "Appearance"),
    ("Application", "Application"),
    ("Appliquer", "Apply"),
    ("Appliquer les biffures", "Apply redactions"),
    ("Appliquer les biffures ?", "Apply redactions?"),
    ("Après la page", "After page"),
    ("Assembler (pages, signets)", "Assemble (pages, bookmarks)"),
    ("Au début", "At the start"),
    ("Aucun cadre", "No border"),
    ("Aucun commentaire", "No comments"),
    ("Aucun commentaire ne correspond", "No comment matches"),
    ("Aucun document — Ctrl+O pour ouvrir, ou déposez un PDF ici", "No document — Ctrl+O to open, or drop a PDF here"),
    ("Aucun fichier. Ajoutez-en, ou déposez-les ici : PDF, images, textes.", "No files yet. Add some, or drop them here: PDFs, images, text."),
    ("Aucun fond", "No background"),
    ("Aucun remplissage", "No fill"),
    ("Aucun résultat", "No results"),
    ("Aucun statut", "No status"),
    ("Aucune commande", "No matching command"),
    ("Aucune vue précédente", "No previous view"),
    ("Aucune vue suivante", "No next view"),
    ("Auteur", "Author"),
    ("Auteur inconnu", "Unknown author"),
    ("Avant la page", "Before page"),
    ("Barre des outils", "Tools pane"),
    ("Barrer", "Strike out"),
    ("Barrer et commenter", "Strike out and comment"),
    ("Barrer la sélection", "Strike out the selection"),
    ("Basse résolution", "Low resolution"),
    ("Biffer", "Redact"),
    ("Biffure", "Redaction"),
    ("Biffure : appliquer définitivement", "Redaction: apply permanently"),
    ("Biffure : marquer la sélection", "Redaction: mark the selection"),
    ("Bleu", "Blue"),
    ("Bon", "Good"),
    ("Cadre", "Border"),
    ("Calques (afficher ou masquer le contenu optionnel)", "Layers (show or hide optional content)"),
    ("Caractères remplacés par « ? » (police standard) : {}", "Characters replaced with “?” (standard font): {}"),
    ("Ce document contient des champs à remplir.", "This document has fields to fill in."),
    ("Ce document est déjà protégé", "This document is already protected"),
    ("Ce fichier est protégé par un mot de passe : ouvrez-le d'abord dans un onglet pour le saisir, puis recommencez.", "This file is password-protected: open it in a tab first to enter its password, then try again."),
    ("Ce mot de passe lève les restrictions ; lui seul permet de changer la protection.", "This password lifts the restrictions; only it can change the protection."),
    ("Ce n'est pas le mot de passe des permissions", "This is not the permissions password"),
    ("Cette occurrence coupe une ligature : cherchez le mot entier pour la remplacer.", "This match splits a ligature: search for the whole word to replace it."),
    ("Cette occurrence est à cheval sur deux lignes : modifiez-la dans l'éditeur de texte.", "This match spans two lines: change it in the text editor."),
    ("Changer la protection", "Change the protection"),
    ("Changer sa protection, ou la retirer ?", "Change its protection, or remove it?"),
    ("Chaque champ reprend sa valeur par défaut, ou redevient vide. Ctrl+Z annule.", "Every field goes back to its default value, or becomes empty. Ctrl+Z undoes it."),
    ("Chaque partie devient un fichier PDF, dans le dossier que vous choisirez ensuite.", "Each part becomes a PDF file, in the folder you choose next."),
    ("Chemin copié", "Path copied"),
    ("Chercher au démarrage", "Check at startup"),
    ("Chiffrement AES-256. Notez vos mots de passe : un mot de passe perdu ne se retrouve pas.", "AES-256 encryption. Write your passwords down: a lost password cannot be recovered."),
    ("Choisir le niveau de zoom", "Choose the zoom level"),
    ("Choisir le niveau de zoom…", "Choose the zoom level…"),
    ("Choisir un tampon", "Choose a stamp"),
    ("Choisir un élément posé, le déplacer, le redimensionner", "Pick a placed item, move it, resize it"),
    ("Choisir une image…", "Choose an image…"),
    ("Clair", "Light"),
    ("Cliquez dans le texte, à l'endroit de l'insertion.", "Click in the text where the insertion goes."),
    ("Cliquez le point à désigner, glissez jusqu'à la zone, puis tapez.", "Click the point to show, drag to where the box goes, then type."),
    ("Cliquez ou tracez une zone, puis tapez ; Échap termine.", "Click or drag a box, then type; Esc finishes."),
    ("Cliquez sur la page et tapez", "Click the page and type"),
    ("Cliquez sur la page pour poser l'image ; Échap pour renoncer.", "Click the page to place the image; Esc to cancel."),
    ("Cliquez sur la page pour poser le tampon ; Maj+clic pour en poser d'autres.", "Click the page to place the stamp; Shift+click to place more."),
    ("Cliquez sur la page, à l'endroit de la note.", "Click the page where the note goes."),
    ("Coche", "Check"),
    ("Cocher le commentaire", "Check comment"),
    ("Coché", "Checked"),
    ("Coller une image", "Paste image"),
    ("Combinaison en cours…", "Combining…"),
    ("Combinaison impossible", "Cannot combine"),
    ("Combiner", "Combine"),
    ("Combiner des fichiers", "Combine files"),
    ("Combiner des fichiers…", "Combine files…"),
    ("Commentaire :", "Comment:"),
    ("Commentaire coché", "Comment checked"),
    ("Commentaire décoché", "Comment unchecked"),
    ("Commentaire supprimé — Ctrl+Z le rétablit", "Comment deleted — Ctrl+Z brings it back"),
    ("Commenter", "Comment"),
    ("Confirmez", "Confirm"),
    ("Conserver les formulaires", "Keep form fields"),
    ("Convertisseur PDF", "PDF producer"),
    ("Copier", "Copy"),
    ("Copier la sélection", "Copy the selection"),
    ("Copier le chemin", "Copy path"),
    ("Copier le chemin du fichier", "Copy the file path"),
    ("Copier le texte et les images", "Copy text and images"),
    ("Couleur", "Colour"),
    ("Couleur du texte", "Text color"),
    ("Couleur du trait", "Line color"),
    ("Courier gras", "Courier Bold"),
    ("Crayon", "Pencil"),
    ("Croix", "Cross"),
    ("Création impossible", "Cannot create the document"),
    ("Créer un paraphe", "Create initials"),
    ("Créer une signature", "Create a signature"),
    ("Créé le", "Created"),
    ("Ctrl+molette", "Ctrl+wheel"),
    ("Dans un seul fichier", "In one file"),
    ("Date", "Date"),
    ("Depuis une image…", "From an image…"),
    ("Depuis « {} » — {} page(s)", "From “{}” — {} page(s)"),
    ("Dernière page", "Last page"),
    ("Descendre", "Move down"),
    ("Dessin", "Drawing"),
    ("Dessiner", "Draw"),
    ("Dessinez à main levée ; Échap ou Entrée termine le dessin.", "Draw freehand; Esc or Enter finishes the drawing."),
    ("Disposition : {} — cliquer pour passer à « {} »", "Layout: {} — click to switch to “{}”"),
    ("Disposition des pages", "Page layout"),
    ("Document", "Document"),
    ("Document protégé", "Protected document"),
    ("Document protégé : {}", "Protected document: {}"),
    ("Documents récents", "Recent documents"),
    ("Dossier", "Folder"),
    ("Dupliquer", "Duplicate"),
    ("Dupliquer les pages", "Duplicate pages"),
    ("Déplacer", "Move"),
    ("Effacer", "Clear"),
    ("Effacer le formulaire", "Clear the form"),
    ("Effacer le formulaire ?", "Clear the form?"),
    ("Ellipse", "Ellipse"),
    ("Encre", "Ink"),
    ("English", "English"),
    ("Enregistrement impossible", "Could not save"),
    ("Enregistrer", "Save"),
    ("Enregistrer les modifications ?", "Save changes?"),
    ("Enregistrer sous", "Save as"),
    ("Entrée : suivante · Maj+Entrée : précédente", "Enter: next · Shift+Enter: previous"),
    ("Exiger un mot de passe pour ouvrir le document", "Require a password to open the document"),
    ("Export interdit", "Export not allowed"),
    ("Exporter", "Export"),
    ("Exporter (page web, Word, Excel, images, texte)", "Export (web page, Word, Excel, images, text)"),
    ("Extraction impossible", "Cannot extract"),
    ("Extraction interdite", "Extraction not allowed"),
    ("Extraction pour l'accessibilité", "Extraction for accessibility"),
    ("Extraire", "Extract"),
    ("Extraire les pages dans un fichier", "Extract pages to a file"),
    ("Extraire {} pages", "Extract {} pages"),
    ("Extraire…", "Extract…"),
    ("Faible", "Weak"),
    ("Faire pivoter la vue", "Rotate the view"),
    ("Faire pivoter la vue à droite", "Rotate the view clockwise"),
    ("Faire pivoter la vue à gauche", "Rotate the view counter-clockwise"),
    ("Faites glisser de l'origine vers la pointe ; Maj pour un angle de 45°.", "Drag from the start to the tip; Shift for 45° angles."),
    ("Faites glisser sur la page ; Maj pour un angle de 45°.", "Drag on the page; Shift for 45° angles."),
    ("Faites glisser sur la page ; Maj pour un carré.", "Drag on the page; Shift for a square."),
    ("Faites glisser sur la page ; Maj pour un cercle.", "Drag on the page; Shift for a circle."),
    ("Faites glisser sur le texte à barrer.", "Drag over the text to strike out."),
    ("Faites glisser sur le texte à biffer, puis « Appliquer les biffures ».", "Drag over the text to redact, then “Apply redactions”."),
    ("Faites glisser sur le texte à remplacer.", "Drag over the text to replace."),
    ("Faites glisser sur le texte à souligner d'un trait ondulé.", "Drag over the text to underline with a squiggly line."),
    ("Faites glisser sur le texte à souligner.", "Drag over the text to underline."),
    ("Faites glisser sur le texte à surligner.", "Drag over the text to highlight."),
    ("Fermer", "Close"),
    ("Fermer l'onglet", "Close the tab"),
    ("Fermer la recherche (Échap)", "Close search (Esc)"),
    ("Fermer les autres onglets", "Close other tabs"),
    ("Feutre", "Marker"),
    ("Fichier", "File"),
    ("Fichier introuvable", "File not found"),
    ("Fichier protégé", "Protected file"),
    ("Fichiers combinés", "Combined files"),
    ("Fichiers ignorés", "Files skipped"),
    ("Filtrer", "Filter"),
    ("Filtrer ({})", "Filter ({})"),
    ("Fin", "Thin"),
    ("Flèche", "Arrow"),
    ("Fond", "Background"),
    ("Forme", "Shape"),
    ("Fort", "Strong"),
    ("Fractionner", "Split"),
    ("Fractionner le document", "Split document"),
    ("Fractionner le document…", "Split document…"),
    ("Fractionner par nombre de pages", "Split by page count"),
    ("Fractionner par taille", "Split by size"),
    ("Français", "French"),
    ("Haute résolution", "High resolution"),
    ("Helvetica gras", "Helvetica Bold"),
    ("Historique vidé", "History cleared"),
    ("Ignoré(s), ni PDF, ni image, ni texte : {}", "Skipped, neither PDF, image nor text: {}"),
    ("Il donne tous les droits sur ce document :", "It grants every right over this document:"),
    ("Il faut le mot de passe des permissions.", "The permissions password is needed."),
    ("Image ajoutée — glissez-la ou tirez ses poignées", "Image added — drag it or pull its handles"),
    ("Image convertie en PDF ({} page(s)) — Ctrl+S pour l'enregistrer", "Image converted to PDF ({} page(s)) — Ctrl+S to save it"),
    ("Image illisible", "Unreadable image"),
    ("Image {} · {}", "{} image · {}"),
    ("Importer", "Import"),
    ("Impression", "Printing"),
    ("Impression interdite", "Printing not allowed"),
    ("Imprimer", "Print"),
    ("Imprimer…", "Print…"),
    ("Insertion", "Insertion"),
    ("Insertion impossible", "Cannot insert"),
    ("Installer", "Install"),
    ("Insérer", "Insert"),
    ("Insérer des pages", "Insert pages"),
    ("Insérer des pages…", "Insert pages…"),
    ("Insérer du texte", "Insert text"),
    ("Insérer du texte ici", "Insert text here"),
    ("Insérer les pages d'un fichier", "Insert pages from a file"),
    ("Insérer une page vierge", "Insert blank page"),
    ("Insérer une page vierge après", "Insert blank page after"),
    ("Insérer une page vierge avant", "Insert blank page before"),
    ("Jamais", "Never"),
    ("Joindre un fichier", "Attach a file"),
    ("Joindre un fichier au document", "Attach a file to the document"),
    ("La liste des documents récents sera effacée. Les fichiers eux-mêmes ne sont pas touchés.", "The list of recent documents will be cleared. The files themselves are not touched."),
    ("La page du fichier", "The file's only page"),
    ("La page sera retirée du document. Ctrl+Z la rétablit ; Ctrl+S enregistre.", "The page will be removed from the document. Ctrl+Z brings it back; Ctrl+S saves."),
    ("La page {} prend le contenu de la première page de « {} ». Ses signets, liens et commentaires restent ; Ctrl+Z annule.", "Page {} takes the content of the first page of “{}”. Its bookmarks, links and comments stay; Ctrl+Z undoes it."),
    ("La recherche automatique est désactivée : Acrux ne contacte rien au démarrage.", "Automatic checking is off: Acrux contacts nothing at startup."),
    ("Langue", "Language"),
    ("Le contenu couvert par les marques sera supprimé définitivement du document.", "Content covered by the marks will be permanently removed from the document."),
    ("Le document est fractionné tel qu'il s'affiche, modifications comprises.", "The document is split as displayed, changes included."),
    ("Le fichier choisi n'a que {} page(s) pour {} page(s) à remplacer.", "The chosen file has only {} page(s) for {} page(s) to replace."),
    ("Le mot de passe des permissions doit différer de celui d'ouverture", "The permissions password must differ from the open password"),
    ("Le presse-papiers ne contient ni image ni texte", "The clipboard holds neither an image nor text"),
    ("Le presse-papiers ne contient pas d'image", "The clipboard holds no image"),
    ("Le travail n'a pas pu démarrer.", "The task could not start."),
    ("Le travail s'est interrompu.", "The task stopped unexpectedly."),
    ("Les champs deviennent du contenu fixe des pages : ils ne pourront plus être remplis. Ctrl+Z annule tant que le document n'est pas enregistré.", "The fields become fixed page content: they can no longer be filled in. Ctrl+Z undoes it until the document is saved."),
    ("Les fichiers sont réunis dans l'ordre de la liste : glissez une ligne, ou Ctrl+↑ et Ctrl+↓, pour le changer.", "Files are joined in list order: drag a row, or use Ctrl+↑ and Ctrl+↓, to change it."),
    ("Les permissions de ce document interdisent d'en extraire le contenu. Le mot de passe des permissions lève cette restriction.", "This document's permissions do not allow extracting its content. The permissions password lifts this restriction."),
    ("Les permissions de ce document interdisent l'impression. Le mot de passe des permissions lève cette restriction.", "This document's permissions do not allow printing. The permissions password lifts this restriction."),
    ("Les permissions de ce fichier interdisent d'en extraire les pages.", "This file's permissions forbid extracting its pages."),
    ("Les permissions de « {} » interdisent d'en extraire les pages. Ouvrez-le dans un onglet avec le mot de passe des permissions, puis recommencez.", "The permissions of “{}” forbid extracting its pages. Open it in a tab with the permissions password, then try again."),
    ("Les permissions du document interdisent cette modification. Le mot de passe des permissions lève cette restriction.", "The document's permissions do not allow this change. The permissions password lifts this restriction."),
    ("Les restrictions exigent un mot de passe des permissions", "Restrictions need a permissions password"),
    ("Les {} pages du fichier", "All {} pages of the file"),
    ("Les {} prennent le contenu des {} premières pages de « {} ». Leurs signets, liens et commentaires restent ; Ctrl+Z annule.", "The {} take the content of the first {} pages of “{}”. Their bookmarks, links and comments stay; Ctrl+Z undoes it."),
    ("Les {} s'écrivent dans un nouveau fichier, ou chacune dans le sien. Le document ne change pas.", "The {} are written to one new file, or each to its own. The document does not change."),
    ("Les {} seront retirées du document. Ctrl+Z les rétablit ; Ctrl+S enregistre.", "The {} will be removed from the document. Ctrl+Z brings them back; Ctrl+S saves."),
    ("Ligne", "Line"),
    ("Lire, modifier, remplir et signer un PDF.", "Read, edit, fill in and sign a PDF."),
    ("Légende", "Callout"),
    ("Marques", "Marks"),
    ("Masquer le panneau latéral", "Hide side panel"),
    ("Masquer les outils", "Hide tools"),
    ("Mises à jour", "Updates"),
    ("Modification interdite", "Change not allowed"),
    ("Modifier", "Edit"),
    ("Modifier : objets de la page", "Edit: page objects"),
    ("Modifier le PDF", "Edit PDF"),
    ("Modifier le contenu", "Change the content"),
    ("Modifier le texte", "Edit text"),
    ("Modifier le texte du commentaire…", "Edit comment text…"),
    ("Modifier le texte sélectionné", "Edit the selected text"),
    ("Modifier le texte…", "Edit text…"),
    ("Modifier les objets", "Edit objects"),
    ("Modifié le", "Modified"),
    ("Modèle 3D ({} triangles) — glisser pour tourner, molette pour zoomer, Échap pour refermer", "3D model ({} triangles) — drag to rotate, wheel to zoom, Esc to close"),
    ("Monter", "Move up"),
    ("Mot de passe", "Password"),
    ("Mot de passe d'ouverture", "Password to open"),
    ("Mot de passe des permissions", "Permissions password"),
    ("Mot de passe incorrect", "Incorrect password"),
    ("Mot de passe pour « {} » :", "Password for “{}”:"),
    ("Mot entier", "Whole word"),
    ("Mots-clés", "Keywords"),
    ("Moyen", "Medium"),
    ("Ne pas enregistrer", "Don't save"),
    ("Noir", "Black"),
    ("Nombre de pages par fichier :", "Pages per file:"),
    ("Non", "No"),
    ("Note", "Note"),
    ("Nouveau PDF depuis le presse-papiers", "New PDF from clipboard"),
    ("Nouveau PDF vierge", "New blank PDF"),
    ("Nouveau PDF — Ctrl+S pour l'enregistrer", "New PDF — Ctrl+S to save it"),
    ("Nouveau texte", "New text"),
    ("Nouveau texte :", "New text:"),
    ("Nouvelle note", "New note"),
    ("Numéro de page : de 1 à {}", "Page number: from 1 to {}"),
    ("Nécessaire pour changer la protection :", "Needed to change the protection:"),
    ("Nécessaire pour retirer la protection :", "Needed to remove the protection:"),
    ("OK", "OK"),
    ("Occurrence précédente", "Previous match"),
    ("Occurrence précédente (Maj+F3)", "Previous match (Shift+F3)"),
    ("Occurrence suivante", "Next match"),
    ("Occurrence suivante (F3)", "Next match (F3)"),
    ("Onglet précédent", "Previous tab"),
    ("Onglet suivant", "Next tab"),
    ("Opacité", "Opacity"),
    ("Organiser : Ctrl+clic et Maj+clic sélectionnent, un glisser déplace, R pivote, Suppr supprime", "Organize: Ctrl+click and Shift+click select, drag to move, R rotates, Del deletes"),
    ("Organiser les pages", "Organize pages"),
    ("Outil biffure", "Redaction tool"),
    ("Outil crayon", "Pencil tool"),
    ("Outil ellipse", "Ellipse tool"),
    ("Outil flèche", "Arrow tool"),
    ("Outil insertion de texte", "Insert text tool"),
    ("Outil ligne", "Line tool"),
    ("Outil légende", "Callout tool"),
    ("Outil note", "Note tool"),
    ("Outil rectangle", "Rectangle tool"),
    ("Outil remplacement de texte", "Replace text tool"),
    ("Outil soulignement", "Underline tool"),
    ("Outil soulignement ondulé", "Squiggly underline tool"),
    ("Outil surligneur", "Highlighter tool"),
    ("Outil texte barré", "Strikeout tool"),
    ("Outil zone de texte", "Text box tool"),
    ("Outils de commentaire", "Comment tools"),
    ("Ouvrir", "Open"),
    ("Ouvrir le dossier du fichier", "Show in folder"),
    ("Ouvrir un document", "Open a document"),
    ("Ouvrir un document…", "Open a document…"),
    ("PDF créé depuis le presse-papiers — Ctrl+S pour l'enregistrer", "PDF created from the clipboard — Ctrl+S to save it"),
    ("Page", "Page"),
    ("Page précédente", "Previous page"),
    ("Page suivante", "Next page"),
    ("Pages", "Pages"),
    ("Pages invalides : écrivez par exemple 1-3, 5 (de 1 à {})", "Invalid pages: write for example 1-3, 5 (from 1 to {})"),
    ("Pages à insérer", "Pages to insert"),
    ("Panneau latéral (vignettes, signets)", "Side panel (thumbnails, bookmarks)"),
    ("Par nombre de pages", "By page count"),
    ("Par signets", "By bookmarks"),
    ("Par taille", "By size"),
    ("Paramètres", "Settings"),
    ("Paramètres…", "Settings…"),
    ("Paraphe", "Initials"),
    ("Passer au thème clair", "Switch to light theme"),
    ("Passer au thème sombre", "Switch to dark theme"),
    ("Permissions", "Permissions"),
    ("Pivoter", "Rotate"),
    ("Pivoter la page", "Rotate the page"),
    ("Pivoter les pages à droite", "Rotate pages clockwise"),
    ("Pivoter les pages à gauche", "Rotate pages counter-clockwise"),
    ("Pivoter à droite", "Rotate clockwise"),
    ("Pivoter à gauche", "Rotate counter-clockwise"),
    ("Pièce jointe", "Attachment"),
    ("Pièces jointes du document", "Document attachments"),
    ("Plein écran", "Full screen"),
    ("Plume", "Fountain pen"),
    ("Point", "Dot"),
    ("Police", "Font"),
    ("Poser une note", "Add a note"),
    ("Poser une note ici", "Add a note here"),
    ("Position dans le document ({} page(s))", "Position in the document ({} page(s))"),
    ("Première page", "First page"),
    ("Presse-papiers", "Clipboard"),
    ("Propriétés du document", "Document properties"),
    ("Protection impossible", "Could not protect the document"),
    ("Protéger", "Protect"),
    ("Protéger par mot de passe", "Protect with a password"),
    ("Protégé", "Protected"),
    ("Publier", "Post"),
    ("Quitter sans enregistrer", "Quit without saving"),
    ("Recherche en cours…", "Checking…"),
    ("Rechercher dans le document", "Search the document"),
    ("Rechercher dans les commentaires", "Search comments"),
    ("Rechercher et remplacer", "Find and replace"),
    ("Rechercher les mises à jour", "Check for updates"),
    ("Rechercher maintenant", "Check now"),
    ("Rectangle", "Rectangle"),
    ("Refaire", "Redo it"),
    ("Refusé", "Rejected"),
    ("Remplacement", "Replacement"),
    ("Remplacement impossible", "Cannot replace"),
    ("Remplacer", "Replace"),
    ("Remplacer des pages", "Replace pages"),
    ("Remplacer des pages…", "Replace pages…"),
    ("Remplacer la page {} ?", "Replace page {}?"),
    ("Remplacer le texte", "Replace text"),
    ("Remplacer le texte sélectionné…", "Replace the selected text…"),
    ("Remplacer le texte…", "Replace text…"),
    ("Remplacer par :", "Replace with:"),
    ("Remplacer par…", "Replace with…"),
    ("Remplacer {} pages ?", "Replace {} pages?"),
    ("Remplacer « {} » par :", "Replace “{}” with:"),
    ("Remplacer…", "Replace…"),
    ("Remplir et signer", "Fill and sign"),
    ("Remplir les cases", "Fill in the boxes"),
    ("Remplir les formulaires", "Fill in forms"),
    ("Remplissage", "Fill"),
    ("Remplissage impossible", "Could not fill in the field"),
    ("Rendu en cours…", "Rendering…"),
    ("Respecter la casse", "Match case"),
    ("Retirer", "Remove"),
    ("Retirer de la liste", "Remove from the list"),
    ("Retirer la protection", "Remove the protection"),
    ("Retirer le statut du commentaire", "Remove comment status"),
    ("Retiré de la liste", "Removed from the list"),
    ("Rond", "Circle"),
    ("Rouge", "Red"),
    ("Répondre", "Reply"),
    ("Répondre au commentaire", "Reply to comment"),
    ("Réponse publiée", "Reply posted"),
    ("Rétablir l'action annulée", "Redo the undone action"),
    ("Saisir le mot de passe", "Enter the password"),
    ("Saisissez le mot de passe d'ouverture", "Enter the password to open the document"),
    ("Sans titre", "Untitled"),
    ("Signature", "Signature"),
    ("Signatures", "Signatures"),
    ("Signer", "Sign"),
    ("Sombre", "Dark"),
    ("Soulignement", "Underline"),
    ("Soulignement ondulé", "Squiggly underline"),
    ("Souligner", "Underline"),
    ("Souligner d'un trait ondulé", "Squiggly underline"),
    ("Souligner d'un trait ondulé et commenter", "Squiggly underline and comment"),
    ("Souligner et commenter", "Underline and comment"),
    ("Souligner la sélection", "Underline the selection"),
    ("Souligner la sélection d'un trait ondulé", "Squiggly-underline the selection"),
    ("Statut", "Status"),
    ("Statut : Accepté", "Status: Accepted"),
    ("Statut : Annulé", "Status: Cancelled"),
    ("Statut : Refusé", "Status: Rejected"),
    ("Statut : Terminé", "Status: Completed"),
    ("Statut : {}", "Status: {}"),
    ("Stylo", "Pen"),
    ("Sujet", "Subject"),
    ("Suppression impossible", "Cannot delete"),
    ("Supprimer", "Delete"),
    ("Supprimer la page {} ?", "Delete page {}?"),
    ("Supprimer le commentaire", "Delete comment"),
    ("Supprimer le commentaire et ses {} réponse(s) ?", "Delete the comment and its {} reply(ies)?"),
    ("Supprimer les pages", "Delete pages"),
    ("Supprimer {} pages ?", "Delete {} pages?"),
    ("Surlignage", "Highlight"),
    ("Surligner", "Highlight"),
    ("Surligner et commenter", "Highlight and comment"),
    ("Surligner la sélection", "Highlight the selection"),
    ("Surligner les champs", "Highlight fields"),
    ("Système", "System"),
    ("Système ({})", "System ({})"),
    ("Sélectionner toutes les pages", "Select all pages"),
    ("Sélectionnez d'abord un commentaire", "Select a comment first"),
    ("Sélectionnez du texte sur une seule page.", "Select text on a single page."),
    ("Sélectionnez du texte, puis choisissez un outil.", "Select some text, then choose a tool."),
    ("Taille", "Size"),
    ("Taille du texte", "Text size"),
    ("Taille maximale de chaque fichier, en Mo (ou « 500 Ko ») :", "Maximum size of each file, in MB (or “500 KB”):"),
    ("Tampon", "Stamp"),
    ("Tampon posé — Ctrl+S pour enregistrer", "Stamp placed — Ctrl+S to save"),
    ("Tamponner", "Stamp"),
    ("Tampons", "Stamps"),
    ("Taper", "Type"),
    ("Tapez une commande", "Type a command"),
    ("Terminer", "Done"),
    ("Terminé", "Completed"),
    ("Texte", "Text"),
    ("Texte barré", "Strikeout"),
    ("Texte de la note (page {}) :", "Note text (page {}):"),
    ("Texte du commentaire", "Comment text"),
    ("Texte du commentaire (page {}) :", "Comment text (page {}):"),
    ("Texte libre", "Free text"),
    ("Texte à ajouter", "Text to add"),
    ("Texte à insérer (page {}) :", "Text to insert (page {}):"),
    ("Texte à répartir dans les {} cases :", "Text to spread over the {} boxes:"),
    ("Thème clair / sombre", "Light / dark theme"),
    ("Times gras", "Times Bold"),
    ("Titre", "Title"),
    ("Tous les auteurs", "All authors"),
    ("Tous les outils", "All tools"),
    ("Tous les types", "All types"),
    ("Tout enregistrer", "Save all"),
    ("Tout remplacer", "Replace all"),
    ("Tout sélectionner", "Select all"),
    ("Toutes (ou par exemple 1-3, 5)", "All (or for example 1-3, 5)"),
    ("Tracer", "Draw"),
    ("Tracez votre signature ici — Ctrl+Z défait le dernier trait", "Draw your signature here — Ctrl+Z undoes the last stroke"),
    ("Tracez à main levée", "Draw freehand"),
    ("Trait", "Line"),
    ("Trier : {}", "Sort: {}"),
    ("Type", "Type"),
    ("Un document doit garder au moins une page.", "A document must keep at least one page."),
    ("Un fichier par page", "One file per page"),
    ("Un nombre entier de pages, 1 ou plus.", "A whole number of pages, 1 or more."),
    ("Un signet par fichier", "One bookmark per file"),
    ("Une page par fichier", "One page per file"),
    ("Une taille comme « 2 », « 2,5 » (Mo) ou « 500 Ko ».", "A size such as “2”, “2.5” (MB) or “500 KB”."),
    ("Valider", "OK"),
    ("Version PDF", "PDF version"),
    ("Version installée : {}.", "Installed version: {}."),
    ("Vert", "Green"),
    ("Vider", "Clear"),
    ("Vider l'historique", "Clear history"),
    ("Votre commentaire", "Your comment"),
    ("Votre nom sera écrit dans une police manuscrite du système.", "Your name will be written in a handwriting font from the system."),
    ("Votre paraphe", "Your initials"),
    ("Votre remarque", "Your remark"),
    ("Votre réponse", "Your reply"),
    ("Votre signature", "Your signature"),
    ("Vue pivotée de {}° (le document n'est pas modifié)", "View rotated {}° (the document is unchanged)"),
    ("Vue pivotée de {}° — cliquer pour la remettre droite", "View rotated {}° — click to straighten it"),
    ("Vue précédente", "Previous view"),
    ("Vue remise droite", "View straightened"),
    ("Vue suivante", "Next view"),
    ("Zone de texte", "Text box"),
    ("Zone de texte (commentaire)", "Text box (comment)"),
    ("Zoom 100 %", "Zoom 100%"),
    ("Zoom arrière", "Zoom out"),
    ("Zoom avant", "Zoom in"),
    ("Zoom limité à {} % pour ce document : au-delà, ses pages ne tiendraient plus en mémoire", "Zoom limited to {}% for this document: beyond that, its pages would not fit in memory"),
    ("accueil", "home"),
    ("assemblage interdit", "assembly not allowed"),
    ("au-delà de 127 octets, la fin du mot de passe est ignorée", "beyond 127 bytes, the end of the password is ignored"),
    ("auto", "auto"),
    ("bloc posé : glissez-le, tirez une poignée, ou cliquez dedans pour écrire", "block placed: drag it, pull a handle, or click inside to type"),
    ("bloc sélectionné : glissez pour le déplacer, les poignées pour le redimensionner, double-cliquez pour écrire", "block selected: drag to move it, the handles to resize it, double-click to type"),
    ("ce bloc ne peut pas être déplacé", "this block cannot be moved"),
    ("ce champ est en lecture seule", "this field is read-only"),
    ("ce document n'a pas de champ de formulaire", "this document has no form field"),
    ("ce document n'a pas encore de fichier : enregistrez-le d'abord", "this document has no file yet: save it first"),
    ("ce document n'est pas protégé", "this document is not protected"),
    ("cette liste n'a aucune option", "this list has no options"),
    ("champs non surlignés", "fields not highlighted"),
    ("champs surlignés", "fields highlighted"),
    ("cliquez dans une ligne de texte", "click inside a line of text"),
    ("commentaires interdits", "commenting not allowed"),
    ("continu", "continuous"),
    ("continu, deux pages", "continuous, two pages"),
    ("copie interdite", "copying not allowed"),
    ("copie interdite par les permissions du document", "copying is not allowed by the document's permissions"),
    ("deux pages", "two pages"),
    ("document protégé : le mot de passe sera demandé à l'ouverture", "document protected: the password will be asked when opening it"),
    ("document protégé : ouverture libre, permissions restreintes", "document protected: opens freely, permissions restricted"),
    ("extraction pour l'accessibilité interdite", "extraction for accessibility not allowed"),
    ("formulaire aplati — Ctrl+Z pour revenir", "form flattened — Ctrl+Z to go back"),
    ("formulaire effacé — Ctrl+Z pour revenir", "form cleared — Ctrl+Z to go back"),
    ("fractionnement en cours vers {}…", "splitting into {}…"),
    ("fractionnement impossible : {}", "cannot split: {}"),
    ("impression en basse résolution seulement", "low-resolution printing only"),
    ("impression interdite", "printing not allowed"),
    ("largeur", "width"),
    ("le texte de cette image n'a pas pu etre relu", "the text of this image could not be read"),
    ("lecture du texte de l'image...", "reading the text in the image..."),
    ("les deux saisies diffèrent", "the two entries differ"),
    ("mises à jour : recherche au démarrage", "updates: checking at startup"),
    ("mises à jour : recherche désactivée", "updates: checking off"),
    ("modification interdite", "changes not allowed"),
    ("modifications non enregistrées", "unsaved changes"),
    ("non", "no"),
    ("ou déposez des PDF ou des images · Ctrl+Maj+P pour toutes les commandes", "or drop PDFs or images · Ctrl+Shift+P for every command"),
    ("oui", "yes"),
    ("oui, tous les droits accordés", "yes, every right granted"),
    ("p. {}", "p. {}"),
    ("page", "page"),
    ("page déplacée en position {}", "page moved to position {}"),
    ("page unique", "single page"),
    ("page vierge insérée en position {} — Ctrl+Z pour revenir", "blank page inserted at position {} — Ctrl+Z to undo"),
    ("page {}", "page {}"),
    ("page {} dupliquée", "page {} duplicated"),
    ("pages {}", "pages {}"),
    ("pages {} à {}", "pages {} to {}"),
    ("protection retirée : enregistrez pour l'appliquer", "protection removed: save to apply it"),
    ("remplacer substituer corriger partout texte", "replace substitute correct everywhere text"),
    ("remplissage des formulaires interdit", "form filling not allowed"),
    ("sur {}", "of {}"),
    ("sélectionnez d'abord du texte", "select some text first"),
    ("texte de l'image reconnu : modifiez-le", "text recognised in the image: edit it"),
    ("tous les droits sont accordés : refaites l'action", "every right is granted: do it again"),
    ("un caractère par case", "one character per box"),
    ("un mot de passe accentué risque d'être refusé par d'autres lecteurs PDF", "a password with accents may be refused by other PDF readers"),
    ("un mot de passe vide ne protège rien", "an empty password protects nothing"),
    ("vue {}°", "view {}°"),
    ("{} (dossier)", "{} (folder)"),
    ("{} de {} — page {}", "{} by {} — page {}"),
    ("{} documents ouverts ont été modifiés. Les enregistrer avant de quitter ?", "{} open documents have been modified. Save them before quitting?"),
    ("{} documents récents", "{} recent documents"),
    ("{} fichier(s)", "{} file(s)"),
    ("{} fichier(s) écrit(s) dans {}", "{} file(s) written to {}"),
    ("{} fichiers combinés ({} pages) — Ctrl+S pour enregistrer", "{} files combined ({} pages) — Ctrl+S to save"),
    ("{} fichiers insérés ({} pages) — Ctrl+Z annule le dernier", "{} files inserted ({} pages) — Ctrl+Z undoes the last one"),
    ("{} occurrence(s) remplacée(s)", "{} occurrence(s) replaced"),
    ("{} occurrence(s) remplacée(s), {} ignorée(s) (à cheval sur deux lignes ou coupant une ligature)", "{} occurrence(s) replaced, {} skipped (spanning two lines or splitting a ligature)"),
    ("{} onglet(s) fermé(s)", "{} tab(s) closed"),
    ("{} onglet(s) modifié(s) laissé(s) ouvert(s) : enregistrez-les d'abord", "{} modified tab(s) left open: save them first"),
    ("{} page(s) choisie(s)", "{} page(s) chosen"),
    ("{} page(s) insérée(s) — Ctrl+Z pour annuler", "{} page(s) inserted — Ctrl+Z to undo"),
    ("{} page(s) remplacée(s) — Ctrl+Z pour revenir", "{} page(s) replaced — Ctrl+Z to undo"),
    ("{} page(s) supprimée(s) — Ctrl+Z pour revenir", "{} page(s) deleted — Ctrl+Z to undo"),
    ("{} page(s) · {}", "{} page(s) · {}"),
    ("{} pages dupliquées", "{} pages duplicated"),
    ("{} pages déplacées en position {}", "{} pages moved to position {}"),
    ("{} pages pivotées", "{} pages rotated"),
    ("{} sur {}", "{} of {}"),
    ("{} trouvée(s), recherche…", "{} found, searching…"),
    ("{} · protégé : mot de passe demandé", "{} · protected: password needed"),
    ("{} écrite(s) dans {}", "{} written to {}"),
    ("{} — page {}", "{} — page {}"),
    ("« {} » a été modifié. Enregistrer les modifications avant de le fermer ?", "“{}” has been modified. Save the changes before closing it?"),
    ("« {} » a été modifié. Enregistrer les modifications avant de quitter ?", "“{}” has been modified. Save the changes before quitting?"),
    ("« {} » est protégé par un mot de passe : passez par « Insérer des pages ».", "“{}” is password-protected: use “Insert pages”."),
    ("« {} » n'est ni un PDF ni une image (PNG, JPEG, BMP, GIF, TIFF).", "“{}” is neither a PDF nor an image (PNG, JPEG, BMP, GIF, TIFF)."),
    ("À la fin", "At the end"),
    ("Échec de la recherche : {}", "Check failed: {}"),
    ("Épais", "Thick"),
    ("Épaisseur du trait", "Line thickness"),
];

#[cfg(test)]
mod tests {
    use super::*;

    /// La langue est **globale** : deux épreuves qui la changent en même
    /// temps se marcheraient dessus. Elles passent donc l'une après l'autre.
    static SEUL: std::sync::Mutex<()> = std::sync::Mutex::new(());

    /// Prend la langue pour soi le temps d'une épreuve.
    fn seul() -> std::sync::MutexGuard<'static, ()> {
        SEUL.lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
    }

    #[test]
    fn la_table_est_triee_et_sans_doublon() {
        for pair in TABLE.windows(2) {
            assert!(
                pair[0].0 < pair[1].0,
                "table désordonnée : « {} » avant « {} »",
                pair[0].0,
                pair[1].0
            );
        }
    }

    #[test]
    fn to_english_ne_depend_pas_de_la_langue_choisie() {
        // Aucun `seul()` : la fonction ne lit pas l'état global, elle rend la
        // même chose pendant qu'une autre épreuve le change.
        assert_eq!(to_english("Terminer"), "Done");
        assert_eq!(to_english("Enregistrer sous"), "Save as");
        assert_eq!(
            to_english("Phrase absente de la table"),
            "Phrase absente de la table"
        );
    }

    /// Les info-bulles de la barre d'outils et des onglets ont leur anglais :
    /// une bascule qui dirait « Passer au thème sombre » à un anglophone
    /// passerait pour un oubli.
    #[test]
    fn les_bulles_de_la_barre_ont_leur_anglais() {
        for (fr, en) in [
            ("Passer au thème sombre", "Switch to dark theme"),
            ("Passer au thème clair", "Switch to light theme"),
            ("Afficher le panneau latéral", "Show side panel"),
            ("Masquer le panneau latéral", "Hide side panel"),
            ("Tous les outils", "All tools"),
            ("Masquer les outils", "Hide tools"),
            ("Fermer l'onglet", "Close the tab"),
            ("modifications non enregistrées", "unsaved changes"),
        ] {
            assert_eq!(to_english(fr), en);
        }
    }

    /// Les tampons, les images et les nouveaux documents : chaque texte de
    /// l'interface a son anglais.
    #[test]
    fn les_tampons_et_les_images_ont_leur_anglais() {
        for fr in [
            "Tamponner",
            "Tampons",
            "Ajouter mon nom et la date",
            "Depuis une image…",
            "Choisir un tampon",
            "Cliquez sur la page pour poser le tampon ; Maj+clic pour en poser d'autres.",
            "Tampon posé — Ctrl+S pour enregistrer",
            "Ajouter une image",
            "Coller une image",
            "Cliquez sur la page pour poser l'image ; Échap pour renoncer.",
            "Image ajoutée — glissez-la ou tirez ses poignées",
            "Image illisible",
            "Le presse-papiers ne contient pas d'image",
            "Le presse-papiers ne contient ni image ni texte",
            "Nouveau PDF vierge",
            "Nouveau PDF depuis le presse-papiers",
            "Nouveau PDF — Ctrl+S pour l'enregistrer",
            "PDF créé depuis le presse-papiers — Ctrl+S pour l'enregistrer",
            "Création impossible",
            "Sans titre",
            "Presse-papiers",
        ] {
            assert_ne!(to_english(fr), fr, "« {fr} » n'a pas de traduction");
        }
    }

    /// Organiser les pages : chaque texte de la sélection des vignettes,
    /// des pages vierges, du remplacement et du fractionnement a son
    /// anglais.
    #[test]
    fn l_organisation_des_pages_a_son_anglais() {
        for fr in [
            "Organiser les pages",
            "Fractionner le document",
            "Par nombre de pages",
            "Par signets",
            "Par taille",
            "Une page par fichier",
            "Supprimer {} pages ?",
            "Remplacer {} pages ?",
            "Extraire {} pages",
            "Dans un seul fichier",
            "Un fichier par page",
            "page vierge insérée en position {} — Ctrl+Z pour revenir",
            "{} fichier(s) écrit(s) dans {}",
            "fractionnement en cours vers {}…",
            "Nombre de pages par fichier :",
            "Insérer une page vierge avant",
            "Insérer une page vierge après",
            "pages {} à {}",
        ] {
            assert_ne!(to_english(fr), fr, "« {fr} » n'a pas de traduction");
        }
    }

    /// Combiner et insérer : la fenêtre, la feuille, leurs messages.
    #[test]
    fn la_combinaison_et_l_insertion_ont_leur_anglais() {
        for fr in [
            "Combiner des fichiers",
            "Combiner des fichiers…",
            "Ajouter des fichiers…",
            "Un signet par fichier",
            "Conserver les formulaires",
            "Combinaison en cours…",
            "Combiner",
            "{} fichiers combinés ({} pages) — Ctrl+S pour enregistrer",
            "Fichiers combinés",
            "Pages à insérer",
            "La page du fichier",
            "Au début",
            "Avant la page",
            "Après la page",
            "À la fin",
            "{} page(s) insérée(s) — Ctrl+Z pour annuler",
            "Insertion impossible",
            "Fichiers ignorés",
            "ou déposez des PDF ou des images · Ctrl+Maj+P pour toutes les commandes",
        ] {
            assert_ne!(to_english(fr), fr, "« {fr} » n'a pas de traduction");
        }
    }

    #[test]
    fn le_francais_se_rend_tel_quel() {
        let _seul = seul();
        apply(Lang::French, Lang::English);
        assert_eq!(tr("Terminer"), "Terminer");
        assert!(!english());
    }

    #[test]
    fn langlais_traduit_ce_quil_connait() {
        let _seul = seul();
        apply(Lang::English, Lang::French);
        assert_eq!(tr("Terminer"), "Done");
        // La relecture : outils, consignes et types du panneau.
        assert_eq!(tr("Remplacer le texte"), "Replace text");
        assert_eq!(tr("Texte barré"), "Strikeout");
        assert_eq!(
            trf("{} de {} — page {}", &[tr("Remplacement"), "Zoé", "3"]),
            "Replacement by Zoé — page 3"
        );
        // Ce qui manque à la table reste lisible.
        assert_eq!(
            tr("Phrase absente de la table"),
            "Phrase absente de la table"
        );
        apply(Lang::French, Lang::French);
    }

    #[test]
    fn auto_suit_le_systeme() {
        let _seul = seul();
        apply(Lang::Auto, Lang::English);
        assert!(english());
        apply(Lang::Auto, Lang::French);
        assert!(!english());
    }

    #[test]
    fn les_trous_se_remplissent_dans_les_deux_langues() {
        let _seul = seul();
        apply(Lang::French, Lang::French);
        assert_eq!(trf("{} documents récents", &["4"]), "4 documents récents");
        apply(Lang::English, Lang::French);
        assert_eq!(trf("{} documents récents", &["4"]), "4 recent documents");
        // Plus de valeurs que de trous : le reste est ignoré sans casse.
        assert_eq!(trf("Terminer", &["x"]), "Done");
        apply(Lang::French, Lang::French);
    }

    #[test]
    fn les_cles_font_laller_retour() {
        for lang in [Lang::Auto, Lang::French, Lang::English] {
            assert_eq!(Lang::from_key(lang.key()), lang);
            assert!(!lang.label().is_empty());
        }
        assert_eq!(Lang::from_key("n'importe quoi"), Lang::Auto);
    }
}
