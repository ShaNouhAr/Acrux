...
fn main() {
    ...
    // logiques) : agrandie, elle se rouvre agrandie — sans quoi elle
    // déborderait de l'écran, sa taille étant celle du bureau entier.
    let prefs = acrux_app::ui::prefs::Prefs::load();
    let (width, height) = prefs.window;
    let maximised = prefs.window_max;
    let viewer = acrux_app::viewer::Viewer::new(initial);
    if let Err(e) = acrux_app::platform::run("Acrux", width, height, maximised, Box::new(viewer)) {
        eprintln!("erreur : {e}");
        std::process::exit(1);
    }
}
